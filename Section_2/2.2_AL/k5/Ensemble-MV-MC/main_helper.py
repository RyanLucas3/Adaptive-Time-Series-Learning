...
def save_models_and_forecasts(AL_output,
    ...
    if LS == 'Ensemble-MV-MC':
        ...
        optimal_model_df.to_csv(f'../../../../../../DataCentre/RSS_AL/k{k}/Learning_Results/{LS}/'
                                + f'Loss_{LS}_p_{p_norm}_Lambda_{L_global_specification[1][1]}_v0_{L_local_specification[1]}_k_{k}_{method}.csv')
        ...
        optimal_model_df['y_star'] = df(
            AL_output[3].values()).set_index(T_test)
        optimal_model_df.to_csv(f'../../../../../../DataCentre/RSS_AL/k{k}/Learning_Results/{LS}/'
                                + f'Loss_{LS}_p_{p_norm}_Lambda_{L_global_specification[1][1]}_k_{k}_{method}_TTT.csv')
